...
def fetch_data_from_table(source_config, table_name):
    try:
        ...
        if source_connection.is_connected():
            ...
            cursor.execute(f"SELECT * FROM {table_name} WHERE disabled = '0' AND updated = '1' AND deleted = '0'")
...
def update_data_in_table(dest_config, table_name, data, primary_key):
    try:
        ...
        if dest_connection.is_connected():
            ...
            if data:
                ...
                for row in data:
                    values = [row[col] for col in columns if col != primary_key]
...
def update_source_table(source_config, table_name, primary_keys):
    try:
        ...
        if source_connection.is_connected():
            ...
            update_query = f"UPDATE {table_name} SET updated = '0' WHERE host_name = %s"
...
if __name__ == "__main__":
    ...
    update_data_in_table(dest_config, dest_table_name, data, primary_key)
