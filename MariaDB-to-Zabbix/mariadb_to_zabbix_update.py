...
def configure_logger(lexa_config):
    script_log=lexa_config['UPDATE_LOG']
...
def get_hosts_from_database(db_config):
    ...
    cursor.execute("SELECT hostid, host_name, visible_name, description, disabled, interface_type, interface_id, interface_ip_address, interface_dns, interface_use_ip, interface_port, interface_default, inserted, updated, deleted, `type`, type_full, name, alias, os, os_full, os_short, serialno_a, serialno_b, tag, asset_tag, macaddress_a, macaddress_b, hardware, hardware_full, software, software_full, software_app_a, software_app_b, software_app_c, software_app_d, software_app_e, contact, location, location_lat, location_lon, notes, chassis, model, hw_arch, vendor, contract_number, installer_name, deployment_status, url_a, url_b, url_c, host_networks, host_netmask, host_router, oob_ip, oob_netmask, oob_router, date_hw_purchase, date_hw_install, date_hw_expiry, date_hw_decomm, site_address_a, site_address_b, site_address_c, site_city, site_state, site_country, site_zip, site_rack, site_notes, poc_1_name, poc_1_email, poc_1_phone_a, poc_1_phone_b, poc_1_cell, poc_1_screen, poc_1_notes, poc_2_name, poc_2_email, poc_2_phone_a, poc_2_phone_b, poc_2_cell, poc_2_screen, poc_2_notes FROM lexa.alldata WHERE updated=1;")
...
def update_host_in_database(db_config, hostname):
    connection = mysql.connector.connect(
        host=db_config['DB_HOST'],
        user=db_config['DB_USER'],
        password=db_config['DB_PASSWORD'],
    ...
    update_query = "UPDATE lexa.alldata SET updated=0 WHERE host_name = '"+ hostname + "'"
...
def update_hosts_in_zabbix(zabbix_config, hosts, db_config):
...
# Invio della richiesta HTTP
        ...
        # Gestione della risposta
        log_message(f"Host {host.host_name} updating response: {response.text}")

        if response.status_code == 200:
            log_message(f"Host {host.host_name} updated successfully")
            # Aggiorna le informazioni di Zabbix nel database
            update_host_in_database(db_config, host.host_name)
        else:
            log_message(f"Failed to update host {host.host_name}: {response.text}")
...
def main():
    ...
    update_hosts_in_zabbix(zabbix_config, hosts, db_config)
